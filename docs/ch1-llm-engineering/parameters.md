...
1. Temperature 调随机性：要稳定可复现（代码、抽取、分类）压到 0.2 以下，要发散创意才往 1.0 调
2. Top-P/Top-K 是另一条"缩小候选范围"的旋钮，平时别动 Top-K，不稳定时把 Top-P 从 0.9 降到 0.7 比硬调 Temperature 更可控
3. 真正天天要设的只有三个：temperature、max_tokens、stream；其余按需才碰
4. seed + 固定参数能让输出尽量可复现，是调试和写测试时的利器
5. 参数只决定"怎么选词"，决定不了"内容对不对"——质量靠 Prompt，别指望调参修正事实错误
