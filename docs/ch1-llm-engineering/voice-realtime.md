...
> 🕐 内容截至 2026-07｜涉及版本：OpenAI GPT-Realtime-2 / Google Gemini 3.1 Flash Live
...
## 2026 现状：实时语音 Agent 进入生产工程学

上面说的"端到端实时语音 API"，2026 年已经从尝鲜变成了生产选型。目前是两强格局：

- **OpenAI GPT-Realtime-2**（2026-05-07 发布）：把 GPT-5 级别的推理放进了语音对话循环，128K 上下文、覆盖 70+ 语言，工具调用/多步任务能力强。定价按音频 token 计（第三方报道约 $32 / $64 每百万 token 输入/输出，具体以官方定价页为准）
- **Google Gemini 3.1 Flash Live**（2026-03-26 发布，[官方博客](https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-1-flash-live/)）：一批高保真音色，**支持视频输入**（能边通话边"看"摄像头/屏幕），单价明显更低，语言覆盖据第三方报道达 90+

**能力边界怎么选**（基于第三方评测，随版本迭代会变化）：

- GPT-Realtime-2 强在**长会话与复杂 agent 场景**：单会话可维持约 60 分钟，多步工具调用、长程任务更稳——客服排障、电话销售这类"要办事"的场景选它
- Gemini 3.1 Flash Live 强在**成本与多模态输入**：视频输入是独一份，但单会话限约 10–30 分钟（据第三方报道），适合短平快的交互和需要"看"的场景

**生产部署的经验法则**：月用量在 1 万分钟以下，用 Vapi、Retell 这类托管语音 agent 平台更划算——它们帮你处理了电话接入、打断、延迟优化这些脏活，按分钟收费；用量上去之后，托管平台的溢价会超过自建成本，这时自己接 Realtime/Live API + 开源的 LiveKit（实时音视频基础设施）更优，控制力也更强。

> ⚠️ **常见误解**："语音 agent 还是个 demo 玩具。" 2026 年的现实是客服、调度、医疗分诊等场景已经在生产环境跑真实流量——但难点不在模型，而在**工程学**：打断处理、噪声环境、延迟预算、按分钟计费的成本控制。这和你写 Web 后端是同一套工程素养，只是载体换成了音频流。
...
下一节：[1.13 自托管推理：把开源模型跑上生产](./self-hosting)
