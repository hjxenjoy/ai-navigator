...
Ollama 偏单请求、并发差、GPU 易空转。生产换 vLLM 这类引擎（连续批处理 + PagedAttention，同卡吞吐高几倍到几十倍，且是 OpenAI 兼容接口）。显存上最易忽略 **KV Cache**——它随并发数和上下文长度涨，只按模型权重估显存会 OOM。→ [1.13](/ch1-llm-engineering/self-hosting)
</details>
