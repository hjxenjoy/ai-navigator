...
| **第 5 章** 深入与落地 | 四大主题的深度+案例+落地方案：**RAG 深入**（进阶检索/向量库/评估防幻觉/文档处理/案例）、**MCP 深入**（能力与SDK/生产远程/选型/案例）、**Skill 开发**（格式/手把手/案例）、**模型微调实战**（流程/云端/本地LoRA/案例） |
| **第 6 章** 提示词工程精通 | 心法/解剖/推理技巧/输出控制/稳定可靠/迭代评估 + **分场景 Playbook**（信息处理/代码/对话客服/RAG·Agent）+ 模型差异与反模式清单 |
